(function () {
  ...
  function highlightCode(line, lang) {
    ...
    // tokenize: strings, comments, then keywords/numbers in remaining text
    var tokens = [];
    ...
    for (var i = 0; i < tokens.length; i++) {
      ...
      if (tok.type === 'comment') {
        ...
        // highlight keywords and numbers within plain code
  ...
  function highlightYaml(text) {
    ...
    var blockIndent = 0;
    var isScript = false;
    var currentLang = 'lua'; // default when lang_script not present
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      // detect new connector (- name:) resets lang to default
      if (/^\s*- name:/.test(raw)) { currentLang = 'lua'; }

      // detect lang_script value
